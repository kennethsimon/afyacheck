...
export function DateRangePicker({
  ...
  // Get initial selected options from URL
  React.useEffect(() => {
    const initialFrom = searchParams.get(fromField);
    const initialTo = searchParams.get(toField);
  ...
  React.useEffect(() => {
    ...
    if (date?.from) {
      newSearchParams.set(fromField, format(date.from, "yyyy-MM-dd"));
    } else {
      newSearchParams.delete(fromField);
    }

    if (date?.to) {
      newSearchParams.set(toField, format(date.to, "yyyy-MM-dd"));
    } else {
      newSearchParams.delete(toField);
    }
  ...
  return (
    <div className=" gap-2">
      <div className="text-xs pt-4 pb-2 font-bold ">{placeholder}</div>
